...
function loadUserCoalition() {
  try {
    const v = parseInt(localStorage.getItem('crc-user-coalition'), 10);
    if (v === 2 || v === 3) userCoalition = v;
  } catch (_) {}
}

function saveUserCoalition() {
  localStorage.setItem('crc-user-coalition', String(userCoalition));
}
...
function loadIffOverrides() {
  try {
    const raw = localStorage.getItem('crc-iff-overrides');
    if (!raw) return;
...
function saveIffOverrides() {
  ...
  localStorage.setItem('crc-iff-overrides', JSON.stringify(obj));
}
